...
%not required as we are determining based on optimality later
P = 0;
...
R = 75;
R_1 = R*lambda*(mu_2)/(lambda_1*mu_2+lambda_2*mu_1);
R_2 = R*lambda*(mu_1)/(lambda_1*mu_2+lambda_2*mu_1);
...
utility_total = zeros(n_hat_range,n2_range);
P_2 = zeros(n_hat_range,n2_range);

%Cycling through different pairs of (n_hat,n2) and finding utility
for n_hat=n_hat_start:n_hat_start+n_hat_range-1
    for n2=n2_start:n2_start+n2_range-1
        ...
        for z=1:10
            ...
            n1 = floor((R_1*mu_1*c_2 + P_w*c_2 - (P_2(n_hat-n_hat_start+1,n2-n2_start+1))*c_2)/(P_w));
            %n1 = floor((R_1*mu_1*mu_tilda*c_2 + P_w*c_2*mu_1 - (P_2(n_hat-n_hat_start+1,n2-n2_start+1))*mu_tilda*c_2)/(P_w*mu_1));
